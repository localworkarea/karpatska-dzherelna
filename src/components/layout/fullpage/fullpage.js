...
class FullPage {
	...
	init() {
		...
		setTimeout(() => {
			...
			if (this.options.bullets) {
				document.documentElement.classList.remove('--bullets-hidden');
			}
	...
	destroy() {
		...
		// 	this.bulletsWrapper.remove();
		// 	this.bulletsWrapper = null;
		// }

		if (this.options.bullets) {
			document.documentElement.classList.add('--bullets-hidden');
		}

		if (this.iOSTouchHandler) {
			document.removeEventListener('touchmove', this.iOSTouchHandler);
			this.iOSTouchHandler = null;
		}
	...
	events() {
		...
		if (isMobile.iOS()) {
			...
			// 		}
			// 	},{ passive: false });
			this.iOSTouchHandler = (e) => {
				if (e.cancelable) {
					e.preventDefault();
				}
			};
		
			document.addEventListener('touchmove', this.iOSTouchHandler, { passive: false });
		}
		this.setEvents();
	}
	setEvents() {
	...
	setBullets() {
			...
			// }
				...
				this.bulletsWrapper = this.wrapper.querySelector(`.${this.options.bulletsClass}`);
...
// Ініціалізація FullPage з підтримкою breakpoint
if (document.querySelector('[data-fls-fullpage]')) {

	const fullpageElement = document.querySelector('[data-fls-fullpage]');

	// breakpoint з атрибуту
	const breakpoint = parseFloat(fullpageElement.dataset.flsFullpageBreakpoint) || null;

	let fpInstance = null;

	// Ініціалізація FullPage
	function initFullpage() {
		if (!fpInstance) {
			fpInstance = new FullPage(fullpageElement);
			window.flsFullpage = fpInstance;
		}
	}
	...
	function checkWidth() {
		...
		// якщо breakpoint не заданий — працюємо як раніше
		if (!breakpoint) {
			initFullpage();
			return;
		}
	...
	// debounce resize (production оптимізація)
	let resizeTimer;
